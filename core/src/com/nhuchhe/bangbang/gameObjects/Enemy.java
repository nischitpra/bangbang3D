...
//    }
    ...
    public boolean isVisibleToPlayer() {
        rayFrom = getPosition();
        rayTo = GameScreen.gameObjectManger.player.getPosition();
        callback.setRayFromWorld(rayFrom);
        callback.setRayToWorld(rayTo);
        callback.setClosestHitFraction(1);

//        GameScreen.debugDrawer.drawLine(rayFrom, rayTo, new Vector3(1, 1, 0));

        GameScreen.world.rayTest(rayFrom, rayTo, callback);
        if (callback.hasHit()) {
